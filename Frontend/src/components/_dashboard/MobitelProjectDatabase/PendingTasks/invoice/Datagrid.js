...
export default function Datagrid({ DropDownValue, ProjectNameDropdownValue }) {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/geInvoice`);
  ...
  const getData = async () => {
    await axiosInstance.get('/invoicSubmissionPendingColumnGet').then((res) => {
      setColumnVisibilityModel(res.data);
    });
  };

  const updateColumn = async () => {
    await axiosInstance.put('/invoicSubmissionPendingColumnEdit', columnVisibilityModel);
  };

  const updateInstallation = async () => {
    await axiosInstance.put('/updateInvoice', tablePass);
  };
