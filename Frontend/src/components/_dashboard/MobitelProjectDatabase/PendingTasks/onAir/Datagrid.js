...
export default function Datagrid({ DropDownValue, ProjectNameDropdownValue }) {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/getOnAir`);
    // console.log(res.data.success[0].mobitelDatabasePropertys);
    setState(res.data.OnAir);
  ...
  const getData = async () => {
    await axiosInstance.get('/onAirPendingColumnGet').then((res) => {
      setColumnVisibilityModel(res.data);
    });
  };

  const updateColumn = async () => {
    await axiosInstance.put('/onAirPendingColumnEdit', columnVisibilityModel);
  };
