...
export default function Datagrid({ DropDownValue, ProjectNameDropdownValue }) {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/getPo`);
    // console.log(res.data.success[0].mobitelDatabasePropertys);
    setState(res.data.Po);
  ...
  const getData = async () => {
    await axiosInstance.get('/poPendingColumnGet').then((res) => {
      setColumnVisibilityModel(res.data);
    });
  };

  const updateColumn = async () => {
    await axiosInstance.put('/poPendingColumnEdit', columnVisibilityModel);
  };

  const updatePo = async () => {
    await axiosInstance.put('/updatePo', tablePass);
  };
