...
export default function Datagrid({ DropDownValue, ProjectNameDropdownValue }) {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/getCommissioning`);
    setState(res.data.commissioning);
  ...
  const getData = async () => {
    await axiosInstance.get('/commissioningPendingColumnGet').then((res) => {
      setColumnVisibilityModel(res.data);
    });
  };

  const updateColumn = async () => {
    await axiosInstance.put('/commissioningPendingColumnEdit', columnVisibilityModel);
  };
